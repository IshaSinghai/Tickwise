    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          ...
          <Link to="/portal/billing" className="mt-4 inline-flex items-center gap-1 text-sm text-primary hover:underline">Manage billing <ArrowRight className="h-3 w-3" /></Link>
        </Card>
      </div>
      ...
      <Card>
        <div className="flex items-center justify-between">
          ...
          <Link to="/portal/keys" className="text-sm text-primary hover:underline">Manage →</Link>
        </div>
...
export function Card({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return <div className={`rounded-xl border border-border/60 bg-surface p-6 shadow-card ${className}`}>{children}</div>;
}
