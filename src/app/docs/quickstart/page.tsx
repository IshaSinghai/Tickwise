...
export const metadata: Metadata = {
  title: "Quickstart — Tickwise docs",
  description: "Sign up, create a key, make your first metered request.",
};
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          First successful call in five minutes. If it takes longer, that’s a docs bug —{" "}
      ...
      <p className="text-sm text-muted-foreground">
        Free accounts get 25,000 units a month and up to 2 keys. Create a <strong>server</strong>{" "}
        key for server-to-server use.
      </p>
      <h2 className="font-display text-2xl font-semibold">2 · Make a request</h2>
      <CodeBlock
        code={`curl https://api.tickwise.io/v1/pools?chain=ethereum&limit=10 \\\n  -H "KC-APIKey: kc_live_…"`}
      />
      ...
      <p className="text-sm text-muted-foreground">
        Every metered response carries usage headers. Use them; don’t poll{" "}
        <code>/account/usage</code> from clients.
      </p>
      <CodeBlock
        lang="http"
        code={`X-Quota-Limit: 500000\nX-Quota-Remaining: 286512\nX-Quota-Reset: 2026-08-01T00:00:00Z\nX-RateLimit-Limit: 25\nX-RateLimit-Remaining: 24`}
      />
    </div>
  );
}
