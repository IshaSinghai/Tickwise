...
export default function PositionsEndpointsPage() {
  return (
    <div className="space-y-6">
      ...
      <CodeBlock
        code={`curl https://api.tickwise.io/v1/positions?chain=ethereum&sort=apr.desc&limit=25 -H "KC-APIKey: …"`}
      />
      <div className="rounded-md border border-border/60 bg-surface p-4 text-sm">
        ...
        <ul className="mt-2 space-y-1 text-muted-foreground">
          <li>
            <code>chain</code> — <code>ethereum</code> or <code>avalanche</code>
          </li>
          <li>
            <code>sort</code> — <code>apr|fee_apr|roi|pnl|value|age.(asc|desc)</code>
          </li>
          <li>
            <code>limit</code>, <code>cursor</code> — pagination
          </li>
          <li>
            <code>risky</code> — include potentially risky pools
          </li>
        </ul>
      </div>
      ...
      <p className="text-sm text-muted-foreground">
        Reconstructed value series. This is <strong>indicative only</strong> — reconstructed from
        transaction amounts and clamped. Don’t frame it to end-users as an audited history.
      </p>
    </div>
  );
}
