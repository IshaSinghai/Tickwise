...
export default function AdminRevenuePage() {
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="rounded-xl border border-border/60 bg-surface p-6 shadow-card">
          <div className="mb-4 text-xs uppercase tracking-widest text-muted-foreground">Monthly · paid vs pending</div>
          <div className="flex h-48 items-end gap-3">
            {data.map((d) => (
              <div key={d.m} className="flex flex-1 flex-col items-center gap-2">
                <div className="flex w-full flex-col" style={{ height: `${((d.paid + d.pending) / max) * 100}%` }}>
                  <div className="w-full flex-1 rounded-t-md bg-warning/50" title={`Pending ${d.pending}`} style={{ flexGrow: d.pending }} />
        ...
        <div className="rounded-xl border border-border/60 bg-surface p-6 shadow-card">
          ...
          <table className="w-full text-sm">
            <thead className="text-left text-[10px] uppercase tracking-widest text-muted-foreground">
              <tr><th className="py-2">Plan</th><th className="py-2">Active subs</th><th className="py-2">MRR</th></tr>
            </thead>
            <tbody className="divide-y divide-border/60">
              {PLANS.filter(p => p.monthlyPrice > 0).map((p, i) => (
                <tr key={p.id}>
                  ...
                  <td className="py-3 font-mono">${(p.monthlyPrice * (20 - i * 5)).toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
