            ...
            {rows.map((r) => (
              <tr key={r.id}>
                ...
                <td className="px-4 py-3">
                  <span
                    className={`rounded-full border px-2 py-0.5 text-[10px] ${r.status === "active" ? "border-success/40 bg-success/10 text-success" : r.status === "grace" ? "border-warning/40 bg-warning/10 text-warning" : "border-border/60 bg-surface-2 text-muted-foreground"}`}
                  >
                    {r.status}
                  </span>
                </td>
